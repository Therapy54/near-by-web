...
import { createUserWithEmailAndPassword, updateProfile, signInWithPopup, GoogleAuthProvider, type UserCredential } from 'firebase/auth';
import { auth } from '@/lib/firebase';
import Link from 'next/link';

let createSessionCookie = async (idToken: string): Promise<void> => {
  let res = await fetch('http://localhost:4500/api/auth/session', {
    method: 'POST',
...
export default function RegisterPage() {
  ...
  let handleEmailSignup = async (e: React.FormEvent) => {
    ...
    try {
      let credentials: UserCredential = await createUserWithEmailAndPassword(auth, email, password);
